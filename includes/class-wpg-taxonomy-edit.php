...
function cptui_add_styles() {
    ...
    wp_register_script( 'taxonomy-main-js', plugins_url( "../assets/js/wpg.taxonomy.main.js", __FILE__ ), array( 'jquery', 'postbox' ), CPTUI_VERSION, true );
    wp_enqueue_style( 'taxonomy-main-css', plugins_url( "../assets/css/wpg.taxonomy.main.css", __FILE__ ), array(), CPTUI_VERSION );
}
add_action( 'admin_enqueue_scripts', 'cptui_add_styles' );

/**
 * Add our wpg.taxonomy.main.js file, with dependencies on jQuery and jQuery UI.
 *
 * @since 1.0.0
 *
 * @internal
...
function cptui_taxonomies_enqueue_scripts() {
    ...
    if ( ! is_object( $current_screen ) || 'wpg_taxonomies' !== $current_screen->base ) {
        return;
    }
    ...
    wp_localize_script(	'taxonomy-main-js', 'cptui_tax_data',
        array(
...
function addmenu(){
    ...
    add_submenu_page( $parent_slug, __( 'Taxonomies', WPG_TEXT_DOMAIN ), __( 'Taxonomies', WPG_TEXT_DOMAIN ), $capability, 'wpg_taxonomies', 'wpg_taxonomies' );
}
